...
// CORE_MODULES - Production modules (main dashboard)
const CORE_MODULES = [
...
// DEV_MODULES - Developer tools (separate file - DELETE BEFORE PRODUCTION)
const DEV_MODULES = [
  'DeveloperTools.gs'      // Demo mode seeding and clearing
];
...
// ALL_PRODUCTION_MODULES - For validation (includes dev tools)
const ALL_PRODUCTION_MODULES = [...CORE_MODULES, ...DEV_MODULES];
...
function validateModuleDependencies() {
  ...
  const ALL_MODULES = [...ALL_PRODUCTION_MODULES, ...TEST_MODULES];
...
/**
 * Build a single consolidated file from a list of modules
 * @param {Object} options - Build options
 * @param {string[]} options.modules - Array of module filenames
 * @param {string} options.outputFile - Output filename
 * @param {string} options.title - Title for the file header
 * @param {string} options.description - Description for the file header
 * @param {boolean} options.quiet - Suppress verbose output
 * @param {Object} options.versionInfo - Version info object
 * @param {string} options.buildDate - Build date string
 * @returns {Object} Build result with success count, error count, and file size
 */
function buildConsolidatedFile(options) {
  const { modules, outputFile, title, description, quiet, versionInfo, buildDate } = options;
...
`;
  ...
  try {
    ...
    return { successCount, errorCount, errors, size: consolidated.length, outputPath };
  } catch (error) {
    throw new Error(`Error writing ${outputFile}: ${error.message}`);
  }
}
...
function build(options = {}) {
  ...
  const mainModules = includeTests
  ...
  // Step 1: Check for duplicate declarations across ALL modules
  if (!quiet) {
    console.log('🔍 Checking for duplicate declarations...');
  }

  const allModules = [...mainModules, ...DEV_MODULES];
  const duplicateCheck = detectDuplicateDeclarations(allModules);
  ...
  // Step 3: Build main ConsolidatedDashboard.gs
  if (!quiet) {
    console.log('📄 Building ConsolidatedDashboard.gs (main code)...');
  }

  const mainResult = buildConsolidatedFile({
    modules: mainModules,
    outputFile: 'ConsolidatedDashboard.gs',
    title: '509 DASHBOARD - CONSOLIDATED BUILD',
    description: 'Main dashboard code. Copy this file to your Apps Script project.',
    quiet,
    versionInfo,
    buildDate
  });
  ...
  if (!quiet) {
    console.log(`   📦 Size: ${(mainResult.size / 1024).toFixed(2)} KB\n`);
  }

  // Step 4: Build ConsolidatedDeveloperTools.gs (separate file)
  if (!quiet) {
    console.log('📄 Building ConsolidatedDeveloperTools.gs (dev tools)...');
  }

  const devResult = buildConsolidatedFile({
    modules: DEV_MODULES,
    outputFile: 'ConsolidatedDeveloperTools.gs',
    title: 'DEVELOPER TOOLS - DELETE THIS FILE BEFORE PRODUCTION',
    description: 'Demo data seeding and clearing functions.\n * DELETE THIS FILE from Apps Script before going live!',
    quiet,
    versionInfo,
    buildDate
  });
  ...
  if (!quiet) {
    console.log(`   📦 Size: ${(devResult.size / 1024).toFixed(2)} KB\n`);
  }
  ...
  if (!quiet) {
    ...
    console.log(`   📄 ConsolidatedDashboard.gs: ${(mainResult.size / 1024).toFixed(2)} KB (${mainResult.successCount} modules)`);
    console.log(`   📄 ConsolidatedDeveloperTools.gs: ${(devResult.size / 1024).toFixed(2)} KB (${devResult.successCount} modules)`);
    console.log(`   📅 Build date: ${buildDate}`);
    console.log(`   🏷️  Build type: ${buildType}\n`);

    console.log('🎉 Ready for deployment!\n');
    console.log('📋 DEPLOYMENT STEPS:');
    console.log('   1. Copy ConsolidatedDashboard.gs to Apps Script');
    console.log('   2. Copy ConsolidatedDeveloperTools.gs to Apps Script (as separate file)');
    console.log('   3. Use Demo menu to seed test data');
    console.log('   4. BEFORE PRODUCTION: Delete ConsolidatedDeveloperTools.gs\n');
  }
}
...
function verifyModules(includeTests = true) {
  const modules = includeTests
    ? [...CORE_MODULES, ...DEV_MODULES, ...TEST_MODULES]
    : [...CORE_MODULES, ...DEV_MODULES];
 ...
 */
function clean() {
  const files = ['ConsolidatedDashboard.gs', 'ConsolidatedDeveloperTools.gs'];
  let cleaned = 0;
...
function checkDuplicatesOnly() {
  const modules = [...CORE_MODULES, ...DEV_MODULES, ...TEST_MODULES];
